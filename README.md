...
처음 온 팀원은 → **먼저 [§1 설치 & 환경 구성](#1-설치--환경-구성-전-팀원-동일-환경)으로 환경 통일**, 그다음 자기 트랙: **로봇이면 [§3 ROS2 워크스페이스](#3-ros2-워크스페이스-src), FMS면 [§6 FMS 서버](#6-fms-서버-fms_server)**. 통합 계약은 누구나 **[`docs/INTERFACE_CONTRACT.md`](docs/INTERFACE_CONTRACT.md)** 필독.
...
## 1. 설치 & 환경 구성 (전 팀원 동일 환경)

> **이 절만 순서대로 따라가면 모든 PC가 동일 환경이 된다.** 공통 단계 → 역할별 분기(FMS 호스트 / 로봇 유닛) 순서. 처음 세팅하는 팀원은 위에서 아래로 그대로 실행.

### 1.0 전제
- **Ubuntu 22.04 + ROS2 Humble** (TurtleBot4 표준 스택)
- 워크스페이스 경로는 **반드시 `~/alfred_ws`** 로 통일 — `.bashrc`·launch·문서가 이 경로를 가정한다.

### 1.1 저장소 클론 (위치·이름 고정) ⚠️
```bash
# 어떤 PC든 동일하게 — 홈 아래 'alfred_ws' 라는 이름으로 받는다 (레포명 rokey_alfred ≠ 디렉토리명)
git clone https://github.com/sungyu-sung/rokey_alfred.git ~/alfred_ws
cd ~/alfred_ws
```
> 디렉토리명이 다르면(`~/rokey_alfred` 등) 빌드·소싱 경로가 전부 어긋난다. **반드시 `~/alfred_ws`.**
...
sudo rosdep init 2>/dev/null; rosdep update     # rosdep 최초 1회
```
...
rosdep install --from-paths src --ignore-src -r -y   # package.xml 의존성(nav2_msgs·irobot_create_msgs·paho-mqtt 등) 자동 설치
colcon build --symlink-install
```

### 1.4 ~/.bashrc 통일 블록 (1회 추가)
모든 PC가 같은 블록을 갖되, **마지막 두 줄(ROS_DOMAIN_ID·FMS_MQTT_HOST)만 역할에 맞게** 둔다.
```bash
cat >> ~/.bashrc <<'EOF'

# ===== Alfred 워크스페이스 =====
source /opt/ros/humble/setup.bash
[ -f ~/alfred_ws/install/setup.bash ] && source ~/alfred_ws/install/setup.bash
export ROS_DOMAIN_ID=2              # 두 로봇 현재 공유=2 (도메인 분리 시 유닛별 값)
export ROS_LOCALHOST_ONLY=0        # 0 유지 (1이면 로봇·팀원 통신 차단)
export FMS_MQTT_HOST=localhost      # ★ 아래 1.6 참조 — 역할에 맞게 수정
# ==============================
EOF
source ~/.bashrc
```

### 1.5 FMS 파이썬 의존성 (FMS 호스트 PC만)
```bash
python3 -m pip install -r ~/alfred_ws/fms_server/requirements.txt
```

### 1.6 역할별 분기 — 한쪽만 해당
**(A) FMS 호스트 PC** — 브로커를 LAN에 개방 (mosquitto 2.x는 기본 localhost 전용이라 로봇이 못 붙는다)
```bash
sudo tee /etc/mosquitto/conf.d/alfred.conf >/dev/null <<'EOF'
listener 1883 0.0.0.0
allow_anonymous true
EOF
sudo systemctl enable --now mosquitto && sudo systemctl restart mosquitto
ip -4 addr | grep inet      # ← 이 PC의 LAN IP 확인 (로봇들이 FMS_MQTT_HOST로 쓸 값)
```
- `.bashrc`의 `FMS_MQTT_HOST=localhost` 유지. 방화벽 사용 시 `sudo ufw allow 1883/tcp`.

**(B) 로봇 유닛 PC** — 브로커 데몬 불필요(클라이언트만)
- `.bashrc`의 `FMS_MQTT_HOST` 를 **FMS 호스트 PC의 고정 IP**(위 (A) 출력값)로 수정 후 `source ~/.bashrc`.
- 브리지 설정도 동일 IP로: [`docs/bridge_config.template.yaml`](docs/bridge_config.template.yaml) 의 `broker.host`.

### 1.7 설치 검증 (여기까지 통과하면 끝)
```bash
ros2 pkg list | grep alfred         # alfred_* 6개 보이면 빌드 OK
mosquitto_sub -h $FMS_MQTT_HOST -t 'robot/#' -v &   # 브로커 연결 확인(메시지 대기)
# FMS PC라면:
cd ~/alfred_ws/fms_server && python3 tools/integration_test.py   # 정상3+예외4+이상1+API3 전부 PASS
```
...
```
- ⚠️ 호출 시 **PATROL→INTERACTING 전이 필수.** PATROL/IDLE에서 받은 ESCORT task는 거절되므로(§3 거절 규칙), 안 바꾸면 자기 escort를 거절한다.
...
> 최초 설치(클론·의존성·.bashrc·빌드)는 **[§1 설치 & 환경 구성](#1-설치--환경-구성-전-팀원-동일-환경)** 참조. 아래는 일상 재빌드·기동만.

```bash
# 워크스페이스 루트(~/alfred_ws)에서 재빌드
cd ~/alfred_ws
...
> 설치(pip·mosquitto)는 **[§1 설치 & 환경 구성](#1-설치--환경-구성-전-팀원-동일-환경)** 에서 끝냈다는 전제. 아래는 기동만.
```bash
...
# 터미널 ④ 미션 발생 (Interaction 시뮬) — 1층→2층 trans 에스코트
